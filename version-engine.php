...
<body>
	<?php
		...
		function getComments($entry){
			$xml = simplexml_load_file("version/versions/".$entry.".xml");
		...
		function writeNotices(){
			if ($handle = opendir('./version/versions')) {
	    		while (false !== ($entry = readdir($handle))){
	    		    if ($entry != "." && $entry != ".." && $entry != ".htaccess"){
	    		    	$xml = simplexml_load_file("version/versions/".$entry);
	    		    	$time = $xml->time;
						$nin = $xml->nin.".xml";
						$notices["$time"] = $nin;
	    			}
		...
		if ($_POST['action'] == "0"){
			...
			if ($_POST['save'] == "new"){
				include('version/currentNIN.php');
				...
				writeThis($noticeXML, "version/versions/".$NIN.".xml", "w");
				$newNIN = $NIN + 1;
				$ninXML = "<html><body><?php ".'$NIN = '.$newNIN."; ?></body></html>";
				writeThis($ninXML, "version/currentNIN.php", "w");
			}
			else if ($_POST['save'] != ""){
				$xml = simplexml_load_file("version/versions/".$_POST['save'].".xml");
				...
				writeThis($noticeXML, "version/versions/".$_POST['save'].".xml", "w");
			}
			writeNotices();
		}
		else if ($_POST['action'] == "3"){
			unlink("version/versions/".$_POST['save'].".xml");
			writeNotices();
		}
